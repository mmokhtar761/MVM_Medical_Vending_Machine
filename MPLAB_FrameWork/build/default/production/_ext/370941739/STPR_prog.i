# 1 "E:/05- BOSS PROJECT/Project2/MPLAB_FrameWork/02_HAL/STPR_prog.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 288 "<built-in>" 3
# 1 "<command line>" 1
...
const uint8 STPR_arrStpDirPorts[11][2] =
{
    {PORT_A,PORT_A},
    {PORT_E,PORT_E},
    {PORT_D,0xFF},
...
{
    DIO_VidSetPinValue(STPR_arrStpDirPorts[ptrSTPR->UniqueId][0], STPR_arrDirPortPin[ptrSTPR->UniqueId][0],0);;
...
void STPR_voidInitStpr (STPR_type* ptrSTPR, uint8 Copy_UniqueId, uint16 Conpy_stpPerMm, uint16 Copy_stpVel, uint16 stprAccPerInterval)
{
    ...
    arrSTPR_LiveVel[ptrSTPR->UniqueId] = ((uint32)(1000000)/((12500 + 12500)));
...
void STPR_voidMoveStprStps (STPR_type* ptrSTPR, uint16 Copy_steps, STPR_Dir_type Copy_MveDir)
{
    ...
    {
        DIO_VidSetPinValue(STPR_arrStpDirPorts[ptrSTPR->UniqueId][1], STPR_arrDirPortPin[ptrSTPR->UniqueId][1],1);;
    }
    else if (Copy_MveDir == DIR_Low)
    {
        DIO_VidSetPinValue(STPR_arrStpDirPorts[ptrSTPR->UniqueId][1], STPR_arrDirPortPin[ptrSTPR->UniqueId][1],0);;
    }
...
{
    if (ptrSTPR->stprStat != IDLE) return;
    if (Copy_stpVel>(uint16)((uint32)(1000000)/(4 + 4)))
    {
        ptrSTPR->stpVel = (uint16)((uint32)(1000000)/(4 + 4));
        ptrSTPR->stpWidth = (4 + 4) ;
    }
    else if (Copy_stpVel<(uint16)((uint32)(1000000)/(12500 + 12500)))
    {
        ptrSTPR->stpVel = (uint16)((uint32)(1000000)/(12500 + 12500));
        ptrSTPR->stpWidth = (12500 + 12500) ;
    }
    else
    {
        ptrSTPR->stpVel = Copy_stpVel;
        ptrSTPR->stpWidth = (uint32)(1000000)/Copy_stpVel;
    }
}
